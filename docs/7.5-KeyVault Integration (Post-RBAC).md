...
> 🔒 This step must be executed **after AKS creation** and **after assigning RBAC roles** to its Managed Identity.

#### 1️⃣ Assign AKS Managed Identity Access to the Vault
...
#### 2️⃣ Enable the Key Vault CSI Secrets Provider in AKS
...
✅ Once these pods are in `Running` state, the cluster can securely mount Key Vault secrets into pods.
